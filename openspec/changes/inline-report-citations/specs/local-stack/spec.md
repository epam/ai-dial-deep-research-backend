...
The opt-in overlay that adds the next-generation chat (see the two-generations requirement) SHALL pin its two services the same way: a `1.0.0-rc.x`-line `epam/ai-dial-chat` tag and the `epam/ai-dial-chat-themes` tag that line expects. Those two tags SHALL be the only images the repository pins to a pre-release line, and the reason SHALL be recorded where they are pinned: no released chat renders marker-tag annotations yet.
