...
tool**, counted per tool call rather than per iteration. Having spent it, research-agent SHALL
move on.
...
three attempts each therefore invoke the tool up to nine times for one piece of evidence.
...
**Research-review.** Its prompt SHALL state that a plan item may be uncovered because a tool
failed rather than because research-agent skipped it. Evidence missing for that reason SHALL be
treated like any other gap: research-review MAY include the work in the next plan so a later
iteration attempts it again, and SHALL NOT treat a failed tool as coverage.
...
- **WHEN** research-agent has called the same failed tool twice more after its first failure and it fails again
...
#### Scenario: Research-review plans work a failed tool left uncovered

- **WHEN** a plan item is unsupported by the findings because the tool that would have covered it failed
- **THEN** research-review SHALL be able to include that item in the next plan, rather than judging it covered or dropping it silently
