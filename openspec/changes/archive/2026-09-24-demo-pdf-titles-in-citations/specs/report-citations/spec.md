...
research turn labels a document no title resolved for. Because the demo reads no application
properties, the pill SHALL shorten a title to the default of the channel's pill-title budget, as a
channel that leaves that property unset does; the popup card SHALL carry the title whole.
...
- **THEN** every popup entry for those documents SHALL read the whole attachment title followed by
  the cited page, such as `Market Outlook 2025.pdf, page 2`, and every pill SHALL read the title
  shortened to the default pill-title budget followed by the cited page, such as
  `Market Outlook 2025…, page 2`, whatever title the PDFs' own metadata carries
