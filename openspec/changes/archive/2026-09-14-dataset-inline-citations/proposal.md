...
- **One new configuration field, `dataset_metadata_tool`, names that tool on a `statgpt` server,
  and a `statgpt` server must name it.** Only a `statgpt` server may set it, since datasets are what
  such a server serves. It is **required** for the reason `file_sharing_tool` is required of a
  document server: it is the only thing that turns a cited URN into a name and a page the reader can
  open, so a dataset server without it serves datasets that cannot be cited.

- **The document-metadata resource and title key become required of a `generic_rag` server**, by
  the same rule. A document id is internal to the server that issued it, so a channel naming no
  resource labels every pill `doc <id>, page <ix>` — a number the reader cannot place. What stays
  optional is the *answer*: an id the resource does not know, or a document with nothing usable
  under the configured key, still costs that citation its title and nothing else.

  **This is a breaking configuration change.** A channel that has not set the fields its servers'
  types require fails validation, and its turns are delivered as "application not configured" until
...
- `application-config-schema`: the new `dataset_metadata_tool` field on an MCP server entry, its
  `statgpt`-only restriction, and the rule that a `statgpt` server must name it; and the
  document-metadata resource and title key, which a `generic_rag` server must now name for the same
  reason.
...
- **`src/dial_deep_research/app_properties.py`**: the `dataset_metadata_tool` field on
  `MCPClientSettings`, a validator requiring it on a `statgpt` server and refusing it on every other
  type, the matching rule that a `generic_rag` server must name its document-metadata resource and
  title key, and a `dataset_metadata_tool` accessor on `ApplicationProperties` mirroring the
  `file_sharing_tool` one.
...
- **`dial_conf/core/applications-template.json`**: its `generic_rag` server entry names the
  document-metadata resource and title key, which a server entry of that type is now required to
  set; the template carries no dataset server, so it names no dataset-metadata tool.
- **Not touched**: the README environment-variable table, because no environment variable is added;
  nothing about the report's citation *forms*, which are unchanged — only the prompt text describing
  what fills them.
...
- **Servers**: the StatGPT MCP satisfies the new contract as deployed. `url` and `lastUpdated` are
  ...
  those citations as text without failing anything — the tool being configured is what is required,
  not that every record it reports carries a page.
