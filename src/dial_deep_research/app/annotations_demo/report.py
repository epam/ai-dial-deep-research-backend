"""The fixed report the annotations demo replies with, and the fixtures it cites.
...
The report is written against two cited documents, 101 and 102, whose PDFs the demo ships as
fixtures, and a third, 999, which deliberately has none: its citation is the one that keeps its
marker text because no URL resolved for it.
...
# The demo's cited documents and the fixture PDF of each, by the document id its citations carry.
# Document 999 is absent on purpose — see the no-URL case in the report below.
FIXTURES: dict[int, str] = {101: "doc-101.pdf", 102: "doc-102.pdf"}

# Where a contributor puts those files. Kept out of git, so the repository carries no sample
# binaries; the demo names the file and this folder when one is missing.
FIXTURES_DIR = "data/annotations_demo"

# The deepest page any citation names. A fixture with fewer pages leaves a pill scrolling to a
...
Here is a lone citation in a paragraph: it becomes one pill that opens page 1 of the first
document. [doc 101, page 1]
...
once. [doc 101, page 2] [doc 102, page 1], [doc 101, page 2]

Here is the same document and page cited again, in a paragraph of its own: it draws a second
pill here rather than joining the one above. [doc 101, page 2]

- Here is a citation in a list item: it becomes a pill, as in a paragraph. [doc 102, page 2]

Here are two pages of one document, cited in two sentences: each becomes a pill of its own, and
the two open different pages. [doc 102, page 1]. This second one opens page 3. [doc 102, page 3]
...
| --- | --- |
| A citation in a table cell | [doc 101, page 1] |

### Here is a citation in a heading: it keeps its marker text and shows no pill [doc 101, page 1]

Here is a citation of a document no URL resolved for: it keeps its marker text and shows no
pill, so a citation is never lost, only left as it was written. [doc 999, page 1]
